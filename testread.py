...
def calculate_similarity_between_pairs(
    ...
    try:
        ...
        for column in ["ID"] + coin_finds_compare_columns:
        ...
        suspicious_groups["AttributedGroups"] = suspicious_groups["ID"].apply(
        ...
        with tqdm(total=total_pairs, desc="Processing CoinFind pairs") as pbar:
            for idx1, idx2 in pairs:
                ...
                if groups1.empty or groups2.empty:
                    results.append((find1["ID"], find2["ID"], 0))
                    pbar.update(1)
                    continue

                # Compare CoinGroups rows and calculate similarity
                common_rows = pd.merge(
                    groups1[similarity_columns_coin_groups],
                    groups2[similarity_columns_coin_groups],
                    how="inner"
                )
                total_rows = len(groups1) + len(groups2)
                similarity_percentage = (len(common_rows) * 2 / total_rows) * 100 if total_rows > 0 else 0
